#!/usr/bin/env python3
"""
Jelastic Billing Export
-----------------------
Pulls billing data from a Jelastic/Virtuozzo cloud platform,
writes CSV files, and uploads them to an FTP server.

Designed to run headless on a Linux VM via cron or systemd timer.
All configuration lives in config.yaml (or environment variables).
"""

import os
import sys
import csv
import ftplib
import logging
import logging.handlers
import argparse
from datetime import datetime, timedelta
from pathlib import Path

import requests
import yaml


# ---------------------------------------------------------------------------
# Configuration
# ---------------------------------------------------------------------------

def load_config(config_path="config.yaml"):
    """Load YAML config and overlay environment variable overrides."""
    path = Path(config_path)
    if not path.exists():
        print(f"ERROR: Config file not found: {config_path}")
        sys.exit(1)

    with open(path, "r", encoding="utf-8") as f:
        cfg = yaml.safe_load(f)

    # Environment variable overrides (take priority over config file)
    env_overrides = {
        "JELASTIC_API_URL": ("jelastic", "api_url"),
        "JELASTIC_TOKEN":   ("jelastic", "token"),
        "FTP_HOST":         ("ftp", "host"),
        "FTP_USER":         ("ftp", "username"),
        "FTP_PASS":         ("ftp", "password"),
    }
    for env_var, (section, key) in env_overrides.items():
        val = os.environ.get(env_var)
        if val:
            cfg[section][key] = val

    return cfg


def setup_logging(cfg):
    """Configure rotating file + console logging."""
    log_cfg = cfg.get("logging", {})
    log_file = log_cfg.get("log_file", "./logs/billing_export.log")
    level_str = log_cfg.get("level", "INFO").upper()
    max_bytes = log_cfg.get("max_size_mb", 5) * 1024 * 1024
    backup_count = log_cfg.get("backup_count", 3)

    log_dir = Path(log_file).parent
    log_dir.mkdir(parents=True, exist_ok=True)

    level = getattr(logging, level_str, logging.INFO)
    logger = logging.getLogger("jelastic_billing")
    logger.setLevel(level)

    fmt = logging.Formatter(
        "%(asctime)s [%(levelname)s] %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S"
    )

    # Rotating file handler
    fh = logging.handlers.RotatingFileHandler(
        log_file, maxBytes=max_bytes, backupCount=backup_count, encoding="utf-8"
    )
    fh.setFormatter(fmt)
    logger.addHandler(fh)

    # Console handler
    ch = logging.StreamHandler(sys.stdout)
    ch.setFormatter(fmt)
    logger.addHandler(ch)

    return logger


# ---------------------------------------------------------------------------
# Date range helpers
# ---------------------------------------------------------------------------

def compute_date_range(cfg):
    """Return (start_dt, end_dt) based on config settings."""
    billing = cfg.get("billing", {})
    start_str = billing.get("start_date", "")
    end_str = billing.get("end_date", "")

    if start_str and end_str:
        start_dt = datetime.strptime(start_str, "%Y-%m-%d %H:%M:%S")
        end_dt = datetime.strptime(end_str, "%Y-%m-%d %H:%M:%S")
        return start_dt, end_dt

    lookback = billing.get("lookback", "7d")
    now = datetime.now()
    end_dt = now.replace(hour=23, minute=59, second=59)

    if lookback.endswith("d"):
        days = int(lookback[:-1])
        start_dt = (now - timedelta(days=days)).replace(hour=0, minute=0, second=0)
    elif lookback.endswith("m"):
        months = int(lookback[:-1])
        start_dt = (now.replace(day=1) - timedelta(days=30 * months)).replace(
            hour=0, minute=0, second=0
        )
    else:
        start_dt = (now - timedelta(days=7)).replace(hour=0, minute=0, second=0)

    return start_dt, end_dt


# ---------------------------------------------------------------------------
# Jelastic API calls
# ---------------------------------------------------------------------------

class JelasticClient:
    """Thin wrapper around the Jelastic REST API."""

    def __init__(self, api_url, token, logger):
        self.api_url = api_url.rstrip("/") + "/"
        self.token = token
        self.logger = logger
        self.session = requests.Session()
        self.session.headers.update({"Accept": "application/json"})

    def _call(self, endpoint, params=None):
        """Make an API call and return the parsed JSON response."""
        url = f"{self.api_url}{endpoint}"
        if params is None:
            params = {}
        params["session"] = self.token

        self.logger.debug(f"API call: {endpoint}")
        resp = self.session.get(url, params=params, timeout=60)
        resp.raise_for_status()
        data = resp.json()

        if data.get("result") != 0:
            error_msg = data.get("error", "Unknown error")
            self.logger.error(f"API error on {endpoint}: {error_msg}")
            return None
        return data

    def get_environments(self):
        """Fetch all environments for the authenticated account."""
        data = self._call("environment/control/rest/getenvs")
        if data is None:
            return []
        infos = data.get("infos", [])
        envs = []
        for info in infos:
            env = info.get("env", {})
            envs.append({
                "envName": env.get("envName", ""),
                "appid": env.get("appid", ""),
                "domain": env.get("domain", ""),
                "status": env.get("status", 0),
            })
        self.logger.info(f"Found {len(envs)} environment(s)")
        return envs

    def get_billing_history(self, start_dt, end_dt, period="DAY",
                            target_appid=None, group_nodes=True):
        """
        Fetch billing history. If target_appid is provided, fetches
        billing for that specific environment only.
        """
        start_str = start_dt.strftime("%Y-%m-%d %H:%M:%S")
        end_str = end_dt.strftime("%Y-%m-%d %H:%M:%S")

        params = {
            "startTime": start_str,
            "endTime": end_str,
            "period": period,
            "groupNodes": str(group_nodes).lower(),
        }

        # Try environment-specific billing endpoint first
        if target_appid:
            params["targetAppid"] = target_appid
            data = self._call(
                "billing/account/rest/getaccountbillinghistorybyperiod",
                params
            )
            if data is not None:
                return data.get("array", data.get("object", []))

            # Fallback: try the older endpoint name
            self.logger.debug("Trying fallback billing endpoint")
            data = self._call(
                "billing/account/rest/getenvbillinghistorybyperiod",
                params
            )
            if data is not None:
                return data.get("array", data.get("object", []))

        # Fallback: general billing history (all environments combined)
        data = self._call(
            "billing/account/rest/getbillinghistory",
            params
        )
        if data is not None:
            items = data.get("array", [])
            if target_appid and items:
                items = [i for i in items if i.get("appid") == target_appid]
            return items

        return []

    def get_account_billing(self, start_dt, end_dt, period="DAY"):
        """Fetch billing history for the entire account (all environments)."""
        start_str = start_dt.strftime("%Y-%m-%d %H:%M:%S")
        end_str = end_dt.strftime("%Y-%m-%d %H:%M:%S")

        params = {
            "startTime": start_str,
            "endTime": end_str,
            "period": period,
        }

        data = self._call("billing/account/rest/getbillinghistory", params)
        if data is not None:
            return data.get("array", [])
        return []


# ---------------------------------------------------------------------------
# CSV export
# ---------------------------------------------------------------------------

# Common fields returned by Jelastic billing API
CSV_HEADERS = [
    "envName",
    "date",
    "resourceName",
    "cost",
    "nodeType",
    "nodeGroup",
    "appid",
    "note",
]


def format_row(item):
    """Normalize a billing API item into a flat dict for CSV output."""
    row = {}
    for key in CSV_HEADERS:
        val = item.get(key, "")
        if key == "cost" and val != "":
            try:
                val = f"{float(val):.6f}"
            except (ValueError, TypeError):
                pass
        row[key] = val
    return row


def write_csv(data, filepath, logger):
    """Write billing data to a CSV file."""
    out_dir = Path(filepath).parent
    out_dir.mkdir(parents=True, exist_ok=True)

    if not data:
        logger.warning(f"No data to write to {filepath}")
        return False

    # Discover all unique keys across all items (in case API returns extra fields)
    all_keys = list(CSV_HEADERS)
    for item in data:
        for k in item:
            if k not in all_keys:
                all_keys.append(k)

    with open(filepath, mode="w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=all_keys, extrasaction="ignore")
        writer.writeheader()
        for item in data:
            writer.writerow(format_row(item))

    row_count = len(data)
    logger.info(f"CSV written: {filepath} ({row_count} rows)")
    return True


def build_filename(pattern, env_name="", start_dt=None, end_dt=None):
    """Replace placeholders in the filename pattern."""
    now = datetime.now()
    replacements = {
        "{date}": now.strftime("%Y-%m-%d"),
        "{datetime}": now.strftime("%Y-%m-%d_%H%M%S"),
        "{env}": env_name,
        "{start}": start_dt.strftime("%Y-%m-%d") if start_dt else "",
        "{end}": end_dt.strftime("%Y-%m-%d") if end_dt else "",
    }
    result = pattern
    for placeholder, value in replacements.items():
        result = result.replace(placeholder, value)
    return result


# ---------------------------------------------------------------------------
# FTP upload
# ---------------------------------------------------------------------------

def upload_to_ftp(filepath, cfg, logger):
    """Upload a file to the configured FTP server."""
    ftp_cfg = cfg.get("ftp", {})
    if not ftp_cfg.get("enabled", False):
        logger.info("FTP upload disabled in config, skipping")
        return True

    host = ftp_cfg.get("host", "")
    port = ftp_cfg.get("port", 21)
    user = ftp_cfg.get("username", "")
    password = ftp_cfg.get("password", "")
    use_tls = ftp_cfg.get("use_tls", True)
    remote_dir = ftp_cfg.get("remote_dir", "/")

    if not host or host == "YOUR_FTP_HOST":
        logger.warning("FTP host not configured, skipping upload")
        return False

    try:
        if use_tls:
            ftp = ftplib.FTP_TLS()
        else:
            ftp = ftplib.FTP()

        logger.info(f"Connecting to FTP: {host}:{port}")
        ftp.connect(host, port, timeout=30)
        ftp.login(user, password)

        if use_tls:
            ftp.prot_p()

        # Navigate to remote directory (create if needed)
        for part in remote_dir.strip("/").split("/"):
            if not part:
                continue
            try:
                ftp.cwd(part)
            except ftplib.error_perm:
                ftp.mkd(part)
                ftp.cwd(part)

        filename = Path(filepath).name
        with open(filepath, "rb") as f:
            ftp.storbinary(f"STOR {filename}", f)

        ftp.quit()
        logger.info(f"FTP upload successful: {filename} -> {remote_dir}")

        if ftp_cfg.get("delete_after_upload", False):
            Path(filepath).unlink()
            logger.info(f"Local file deleted after upload: {filepath}")

        return True

    except Exception as e:
        logger.error(f"FTP upload failed: {e}")
        return False


# ---------------------------------------------------------------------------
# Main workflow
# ---------------------------------------------------------------------------

def run(config_path="config.yaml"):
    """Main entry point: fetch billing, write CSVs, upload to FTP."""
    cfg = load_config(config_path)
    logger = setup_logging(cfg)

    logger.info("=" * 60)
    logger.info("Jelastic Billing Export - Starting")
    logger.info("=" * 60)

    # Validate token
    token = cfg["jelastic"]["token"]
    if not token or token == "YOUR_TOKEN_HERE":
        logger.error("Jelastic API token not configured. Edit config.yaml or set JELASTIC_TOKEN.")
        return False

    # Compute date range
    start_dt, end_dt = compute_date_range(cfg)
    logger.info(f"Date range: {start_dt} to {end_dt}")

    # Initialize API client
    client = JelasticClient(
        api_url=cfg["jelastic"]["api_url"],
        token=token,
        logger=logger,
    )

    billing_cfg = cfg.get("billing", {})
    csv_cfg = cfg.get("csv", {})
    period = billing_cfg.get("period", "DAY")
    group_nodes = billing_cfg.get("group_nodes", True)
    output_dir = csv_cfg.get("output_dir", "./output")
    pattern = csv_cfg.get("filename_pattern", "billing_{date}.csv")
    per_env = csv_cfg.get("per_environment", False)

    files_written = []

    if per_env:
        # Fetch billing per environment, one CSV each
        envs = client.get_environments()
        if not envs:
            logger.error("No environments found. Check your token and API URL.")
            return False

        for env in envs:
            env_name = env["envName"]
            appid = env["appid"]
            logger.info(f"Fetching billing for environment: {env_name} (appid: {appid})")

            data = client.get_billing_history(
                start_dt, end_dt,
                period=period,
                target_appid=appid,
                group_nodes=group_nodes,
            )

            filename = build_filename(pattern, env_name=env_name,
                                      start_dt=start_dt, end_dt=end_dt)
            # If pattern doesn't include {env}, prefix with env name
            if "{env}" not in csv_cfg.get("filename_pattern", ""):
                name_part = Path(filename).stem
                ext = Path(filename).suffix
                filename = f"{name_part}_{env_name}{ext}"

            filepath = Path(output_dir) / filename
            if write_csv(data, str(filepath), logger):
                files_written.append(str(filepath))
    else:
        # Combined CSV: all environments in one file
        logger.info("Fetching billing for all environments (combined mode)")

        # Try to get billing with environment info
        envs = client.get_environments()
        all_data = []

        if envs:
            for env in envs:
                env_name = env["envName"]
                appid = env["appid"]
                logger.info(f"  Fetching: {env_name}")
                data = client.get_billing_history(
                    start_dt, end_dt,
                    period=period,
                    target_appid=appid,
                    group_nodes=group_nodes,
                )
                # Tag each row with the environment name if not already present
                for item in data:
                    if not item.get("envName"):
                        item["envName"] = env_name
                all_data.extend(data)
        else:
            # Fallback: get account-wide billing
            logger.warning("Could not list environments, fetching account-wide billing")
            all_data = client.get_account_billing(start_dt, end_dt, period=period)

        filename = build_filename(pattern, start_dt=start_dt, end_dt=end_dt)
        filepath = Path(output_dir) / filename

        if write_csv(all_data, str(filepath), logger):
            files_written.append(str(filepath))

    # Upload to FTP
    success_count = 0
    for fpath in files_written:
        if upload_to_ftp(fpath, cfg, logger):
            success_count += 1

    # Summary
    logger.info("-" * 60)
    logger.info(f"Export complete: {len(files_written)} CSV file(s) created")
    if cfg.get("ftp", {}).get("enabled", False):
        logger.info(f"FTP uploads: {success_count}/{len(files_written)} successful")
    logger.info("=" * 60)

    return len(files_written) > 0


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Jelastic Billing Export - Fetch billing data and upload to FTP"
    )
    parser.add_argument(
        "-c", "--config",
        default="config.yaml",
        help="Path to config file (default: config.yaml)"
    )
    args = parser.parse_args()

    ok = run(config_path=args.config)
    sys.exit(0 if ok else 1)
